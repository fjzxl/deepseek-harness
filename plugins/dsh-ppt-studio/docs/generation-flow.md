...
- **分配硬校验**：Σ分配 = contentPages 且覆盖全部部分，多/少/未知 sectionId 均拒绝；总和不符时报错列出当前传入的各部分分配与差距页数（多/少 N 页），模型可直接改参重试（0.10.2）。
...
- **页数硬校验**：Σ内容页 = contentPages（超出立即拒，报错附超出页数与各部分当前页数）；有确认分配时各部分精确匹配——不一致时报错直接写明"部分 sN 需要 X 页（当前传入 Y 页）"与两条可执行路径（为该部分传足 X 个 page 对象重试 / 重调 ppt_pageplan_confirm 改分配），并明示"参数校验拒绝，原样重发不会成功"（0.10.2，修复模型页数不符时原样重发死循环的真实会话事故）。
- **逐页蓝图降级（0.10.3，连败自适应）**：同一部分连续 3 次（DEGRADE_AFTER，与熔断同一 failureStreak 数据源、低于熔断阈值 5）因页数与确认分配不一致被拒后，该部分自动降级为**逐页累积模式**——接受分次提交（每次 1 页也可）、追加而非覆盖（s0 按 type 去重累积）；**只改提交粒度，不改计划语义**：累积总量仍精确等于确认分配，超出剩余槽位照样拒绝并报剩余槽位数；总页数、分配、下游校验（scene_check/design_lock）全部不变。闩锁持久化在 `state.draftPagewise`（quota 快照；否则接受 1 页成功 → 连败清零 → 又回到严格模式拒收，逻辑断裂）；累积满额自动解除恢复整段覆盖语义，重调 ppt_pageplan_confirm 或架构 revise 亦解除（quota 与最新 plan 不符时闩锁作废，双保险）。降级回执带 🔔 用户通知块（"请把本段降级说明原样告知用户"），后续回执报进度"已收 X/N 页"；ppt_deck_status 同步展示逐页模式进度。ppt_pageplan_confirm 不降级（页总数是用户决策，拒绝仍正确）。
- 结构页约束：s0 只允许 cover/toc/closing，且 0.10.1 起必须**一次交齐**各一页（缺页即拒——整段覆盖语义下分次补交会互相覆盖，缺口永远补不齐，design_lock 必然失败；0.10.3 逐页降级模式下放宽为按 type 去重累积，集齐三类解除）；内容部分不得使用 cover/toc 页型。重复调用同一部分为整段覆盖，回执以 `replaced` 字段提示被丢弃的旧页数。
